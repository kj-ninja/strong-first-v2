...
export const changeMonth = (direction) => (dispatch, state) =>{
  console.log(state);
  const newMonth = generateNewMonthDate(direction, state.pickedMonth);
  const monthExist = state.calendarStructure.some((item) => item.month === newMonth);
